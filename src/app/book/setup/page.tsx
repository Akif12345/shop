...
const copy = {
  en: {
    ...
    subtitle: "Book installation support for gyms, homes, or relocations.",
    ...
    fields: {
      ...
      machineBrand: "Machine Brand (optional)",
      machineModel: "Machine Model (optional)",
      machineDetails: "Machine Details (optional)",
      date: "Preferred Date",
    ...
    placeholders: {
      ...
      address: "Full installation address in Riyadh",
      machineBrand: "e.g., Life Fitness, Precor",
      machineModel: "e.g., T5, 95T",
      machineDetails: "Number of machines, type, etc.",
      notes: "Any extra details",
    },
  },
  ar: {
    back: "العودة لبوابة الحجز",
    title: "التركيب / الإعداد",
    subtitle: "احجز خدمة تركيب للأندية أو المنازل أو بعد نقل الأجهزة.",
    submit: "إرسال طلب التركيب",
    ...
    fields: {
      ...
      machineBrand: "ماركة الجهاز (اختياري)",
      machineModel: "موديل الجهاز (اختياري)",
      machineDetails: "تفاصيل الجهاز (اختياري)",
      date: "التاريخ المفضل",
    ...
    placeholders: {
      ...
      phone: "+966 5X XXX XXXX",
      email: "you@example.com",
      address: "عنوان التركيب الكامل في الرياض",
      machineBrand: "مثال: Life Fitness, Precor",
      machineModel: "مثال: T5, 95T",
      machineDetails: "عدد الأجهزة، النوع، إلخ.",
...
export default function SetupBookingPage() {
  ...
  const onSubmit = handleSubmit((data) => {
    ...
    startTransition(async () => {
      const res = await submitServiceRequest("setup", {
        requestType: "setup",
        ...
        locationDetails: `[${locationType}] ${data.address}`,
      ...
      <div className="mx-auto w-full max-w-2xl">
        ...
        <section className="mt-6 rounded-3xl border border-black/10 bg-white p-6 shadow-sm sm:p-8">
          ...
          <form onSubmit={onSubmit} className="mt-6 space-y-5" noValidate>
            ...
            {/* Location Type */}
            <Field label={t.fields.locationType}>
              <div className="flex gap-3">
                {[
                  { value: "gym", label: t.fields.gym },
                  { value: "home", label: t.fields.home },
                  { value: "other", label: t.fields.other },
                ].map((opt) => (
                  <label
                    key={opt.value}
                    className={`flex-1 cursor-pointer rounded-xl border px-3 py-3 text-center text-sm font-medium transition ${
                      locationType === opt.value
                        ? "border-[#d44a1a] bg-[#d44a1a]/10 text-[#d44a1a]"
            ...
            <Field label={t.fields.machineBrand} error={fieldError("machineBrand")}>
              <input
                {...register("machineBrand")}
                type="text"
                placeholder={t.placeholders.machineBrand}
                className="w-full rounded-xl border border-black/15 bg-white px-4 py-3 text-sm focus:border-[#d44a1a] focus:outline-none focus:ring-2 focus:ring-[#d44a1a]/20"
              />
            </Field>

            <Field label={t.fields.machineModel} error={fieldError("machineModel")}>
              <input
            ...
            <Field label={t.fields.machineDetails}>
              <textarea
                {...register("issueDescription")}
                rows={2}
                placeholder={t.placeholders.machineDetails}
                className="w-full rounded-xl border border-black/15 bg-white px-4 py-3 text-sm focus:border-[#d44a1a] focus:outline-none focus:ring-2 focus:ring-[#d44a1a]/20"
              />
            </Field>
