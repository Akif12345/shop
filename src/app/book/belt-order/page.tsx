...
const copy = {
  en: {
    ...
    title: "Belt Order Request",
    subtitle: "Request treadmill belt recommendations and ordering support.",
    submit: "Submit Belt Order",
    submitting: "Submitting...",
    success: "Order request submitted! We will contact you soon.",
    fields: {
      name: "Full Name",
      phone: "Phone Number",
      ...
      brand: "Machine Brand",
      model: "Machine Model",
      beltSize: "Belt Size (if known, optional)",
      quantity: "Quantity",
    ...
    placeholders: {
      ...
      beltSize: "e.g., 42cm x 140cm",
      address: "Full delivery address in Dammam",
      notes: "Any extra details",
    },
  },
  ar: {
    back: "العودة لبوابة الحجز",
    title: "طلب حزام جهاز المشي",
    subtitle: "اطلب ترشيح الحزام المناسب والمساعدة في الطلب.",
    submit: "إرسال طلب الحزام",
    ...
    fields: {
      ...
      brand: "ماركة الجهاز",
      model: "موديل الجهاز",
      beltSize: "مقاس الحزام (إن كان معروفاً، اختياري)",
      quantity: "الكمية",
    ...
    placeholders: {
      ...
      beltSize: "مثال: 42cm x 140cm",
      address: "عنوان التوصيل الكامل في الدمام",
...
export default function BeltOrderPage() {
  ...
  const onSubmit = handleSubmit((data) => {
    ...
    startTransition(async () => {
      const res = await submitServiceRequest("belt_order", {
        requestType: "belt_order",
      ...
      <div className="mx-auto w-full max-w-2xl">
        ...
        <section className="mt-6 rounded-3xl border border-black/10 bg-white p-6 shadow-sm sm:p-8">
          ...
          <form onSubmit={onSubmit} className="mt-6 space-y-5" noValidate>
            ...
            <Field label={t.fields.beltSize} error={fieldError("beltSize")}>
              <input
                {...register("beltSize")}
                type="text"
                placeholder={t.placeholders.beltSize}
                className="w-full rounded-xl border border-black/15 bg-white px-4 py-3 text-sm focus:border-[#d44a1a] focus:outline-none focus:ring-2 focus:ring-[#d44a1a]/20"
              />
            </Field>

            <Field label={t.fields.quantity} error={fieldError("quantity")}>
              <input
                {...register("quantity", { required: true, valueAsNumber: true })}
                type="number"
                min={1}
                className="w-full max-w-32 rounded-xl border border-black/15 bg-white px-4 py-3 text-sm focus:border-[#d44a1a] focus:outline-none focus:ring-2 focus:ring-[#d44a1a]/20"
              />
            </Field>

            <Field label={t.fields.address} error={fieldError("address")}>
              <input
